
## 📌 Descrizione

Mini CRM è una piccola applicazione web per la gestione dei clienti, pensata per **imparare e mostrare le basi di un CRUD** (Create, Read, Update, Delete) tra frontend e backend.  

- I clienti vengono visualizzati in **card moderne e colorate**  
- È possibile **aggiungere nuovi clienti** tramite form  
- Il progetto utilizza **fetch API** per comunicare tra frontend (JavaScript) e backend (PHP/MySQL)  

---

## 📂 Struttura del progetto

mini-crm/
│
├─ backend/
│ ├─ api.php # API per ottenere lista clienti
│ ├─ add_client.php # Inserimento cliente
│ └─ db.php # Connessione al database
│
├─ frontend/
│ ├─ index.html # Pagina principale
│ ├─ app.js # Script JavaScript
│ └─ style.css # Stile e layout card
│
└─ README.md # Questo file

---

## ⚙️ Tecnologie utilizzate

- **Frontend**: HTML5, CSS3, JavaScript (vanilla)  
- **Backend**: PHP  
- **Database**: MySQL / MariaDB  
- **Server locale**: XAMPP 
=======
# mini-crm
Mini CRM è un piccolo progetto web per la gestione di clienti, realizzato con **HTML, CSS, JavaScript (frontend)** e **PHP/MySQL (backend)**.   Ogni cliente può essere aggiunto tramite un form e viene visualizzato in **card colorate** nel frontend.

